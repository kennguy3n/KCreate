...
pub fn vision_start(pack_id: String) -> Phase4Result<u16> {
    ...
    let handle = spawn_vision(&plan)?;
    let port = wait_for_port(&handle)?;
    *guard = Some(handle);
    Ok(port)
}
...
fn spawn_vision(plan: &DispatchPlan) -> Phase4Result<VisionHandle> {
    match plan.runtime {
        SidecarRuntime::LlamaServer => {
            ...
            s.start().map_err(Phase4BridgeError::Sidecar)?;
            Ok(VisionHandle::Llama(s))
        }
        SidecarRuntime::MlxLm => {
            ...
            s.start().map_err(Phase4BridgeError::Sidecar)?;
            Ok(VisionHandle::Mlx(s))
        }
    }
}

/// Block on the sidecar's health-check thread until it transitions
/// out of `Starting`. Returns the listening port on `Ready`,
/// propagates the typed error on `Error`. Caps the wait at the
/// per-runtime health timeout (90 s for VLM / MLX) so the renderer
/// never hangs the UI thread waiting for a misconfigured spawn.
fn wait_for_port(handle: &VisionHandle) -> Phase4Result<u16> {
    let deadline = std::time::Instant::now() + std::time::Duration::from_mins(2);
    loop {
        match handle.status() {
            SidecarStatus::Ready { port, .. } => return Ok(port),
            SidecarStatus::Error { message } => {
                return Err(Phase4BridgeError::Invalid(message));
            }
            _ => {}
        }
        if std::time::Instant::now() >= deadline {
            return Err(Phase4BridgeError::VisionNotReady);
        }
...
pub fn vision_describe_node(node_id: Uuid, user_prompt: String) -> Phase4Result<String> {
...
pub fn vision_generate_alt_text(rgba: Vec<u8>, width: u32, height: u32) -> Phase4Result<String> {
    let port = vision_ready_port()?;
...
pub fn vision_analyze_design(rgba: Vec<u8>, width: u32, height: u32) -> Phase4Result<String> {
    let port = vision_ready_port()?;
