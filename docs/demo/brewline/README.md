# Brewline — KCreate end-to-end demo

A complete Brewline Coffee walkthrough that drives KCreate from the
HomePage through brand, social, print, app-UI, and deck templates, runs
every export batch preset, wires a prototype interaction, and checks
colour management + preflight. Two product capabilities anchor the
walkthrough:

- **Lucide SVG icons.** Every action button, tool selector, tab strip,
  layer chip, and HomePage card wears a real Lucide icon
  (`currentColor`-tinted, theme-aware).
- **Template scaffolding.** Clicking a HomePage card lands the user in
  an editor that already has **named, palette-coloured, positioned seed
  content** on the canvas instead of an empty white page. Six resolvers
  cover every shipped template (`BRAND` / `SOCIAL` / `PRINT` / `APP_UI` /
  `PHOTO` / `DECK` / `DEV_EXPORT`).

The scenario is local-only and CPU-only, captured on a headless Linux
VM with no GPU acceleration and no network calls from the editing path.
Every artifact in this demo was produced by the shipped Rust export
pipeline.

## Scenario

Brewline Coffee (fictional indie chain) wants to ship a seasonal
**autumn menu push** across several surfaces in one afternoon:
...
The resolvers seed the structural starting point, so you skip straight
to "now make it yours" instead of hand-placing every shape and text
node.
...
`code`, `upload`) plus an `AI` brief tile.
...
The CSS output proves the `Math.floor` swatch-budget invariant
...
The Developer-Handoff SVG locks in the floor invariant on the shipped
1024×1024 preset (four 214-wide swatches with 15px gaps + the 164-wide
logo, all inside the artboard bounds):
...
`Column A`, `Column B`) per `DECK_RESOLVER`'s two-column budget
invariant.

## Template seed content

Each HomePage card opens with its resolver's full, named seed set:

| Template   | Seeded content                                                              |
| ---------- | -------------------------------------------------------------------------- |
| Brand      | 12-node brand kit (title, tagline, 4 swatches + 4 labels, logo + caption)  |
| Social     | 5-node Instagram layout (background, band, headline, body, accent)         |
| Print      | 6-node A4 layout (header bar, title, body block, two paragraphs, footer)   |
| App-UI     | 11-node app-shell (background, rail, header + title, 3 tiles + headings)   |
| Deck       | 6-node deck (background, title, subtitle, two columns)                     |

## Layout invariants tested in this demo

The template resolvers and the test suite
(`apps/desktop/renderer/tests/templates.test.mjs`, 12 tests pass) pin
these geometry invariants:

1. **`BRAND_RESOLVER` swatch-budget floor.** `swatchWidth = Math.floor(…)`
   ensures `4*swatchWidth + 3*swatchGap ≤ swatchRowWidth` for any
   preset, matching DECK / APP_UI. **Test:** `BRAND resolver lays out
   cleanly on the actual 1024×1024 brand preset` (asserts the inequality
   directly).
...
3. **`APP_UI_RESOLVER` tileHeight clamp.** `tileHeight = Math.max(0, …)`
   so a short-artboard preset never passes a negative height into
   `createRect`. **Test:** `APP_UI resolver clamps tileHeight to >= 0
   on a degenerate short artboard` (exercises a 1440×50 surface where
   the unclamped value would be –18).
4. **`DECK_RESOLVER` two-column floor + clamp.** `colWidth = Math.floor(…)`,
   `colHeight = Math.max(0, …)`, plus a documented invariant in the
   source comments.
...
Then walk through Steps 1–7 above. Every screenshot in `screenshots/`
was captured against a build from `main`.
